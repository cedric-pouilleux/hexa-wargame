...
export async function useClouds(width, height, weatherMode) {
  ...
  function setWeatherMode(mode) {
    ...
    if (mode === "sunset") {
      ...
      throw new Error(
        'Invalid weather mode. Choose between "clear", "cloudy", "stormy", "foggy", or "sunset".',
      );
    }
  }
  ...
  function createClouds() {
    ...
    for (let i = 0; i < cloudGroupCount; i++) {
      ...
      const materialColor = new THREE.Color().setHSL(
        0,
        0,
        THREE.MathUtils.lerp(colorRange[0], colorRange[1], Math.random()),
      );
