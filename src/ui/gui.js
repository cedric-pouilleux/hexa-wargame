...
export function useGUI({
    ...
    sizeMapGenerateFolder.add(mapConfig, 'rows', 10, 200, 1).name('Rows').onChange(_ => {
        updateWaterCallBack();
        updateMapCallBack();
        updateCloudCallBack();
    });
    sizeMapGenerateFolder.add(mapConfig, 'cols', 10, 200, 1).name('Cols').onChange(_ => {
        updateWaterCallBack();
        updateMapCallBack();
        updateCloudCallBack();
    });
    sizeMapGenerateFolder.add(mapConfig, 'waterHeight', 1, 100, 1).name('Water Height').onChange(_ => {
        updateWaterCallBack();
        updateMapCallBack();
    });
    ...
    noiseMapGenerateFolder.add(mapConfig, 'maxAmplitude', 0, 10, 0.05).name('Max Amplitude').onChange(updateMapCallBack);
